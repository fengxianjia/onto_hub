...
class WebhookService:
    ...
    def get_in_use_package_ids(self, ontology_name: str) -> List[str]:
        """
        获取特定本体名称下正在被 Webhook 使用的包 ID 列表。
        正在使用指：在该 Webhook 下最后一次成功推送的版本。
        """
        webhooks = self.repo.get_webhooks_by_event("ontology.activated", ontology_name=ontology_name)
        in_use_ids = []
        for wh in webhooks:
            latest = self.repo.get_latest_success_delivery(wh.id, ontology_name)
